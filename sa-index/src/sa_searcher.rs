...
impl<SA: SuffixArrayBackend, P: ProteinsBackend, STPM: SuffixToProteinMappingBackend> Searcher<SA, P, STPM> {
    ...
    // ========================================================================
    // MLP cross-query batching (prototype)
    // Interleaves B independent peptide searches so ~B random memory misses are
    // in flight per core, hiding the DRAM latency the scalar dependent-chain
    // binary search cannot. Each stream's per-step logic is identical to the
    // scalar path, so results are identical (see test_batched_matches_scalar).
    // ========================================================================
