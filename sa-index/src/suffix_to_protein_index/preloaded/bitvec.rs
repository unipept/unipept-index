...
pub(super) fn read_bitvec_mapping<R: Read>(reader: &mut R) -> Result<BitVecSuffixToProtein, Box<dyn Error>> {
    ...
    // does the same for the same reason. This is the larger of the structure's two allocations —
    // roughly four fifths of it, one bit per text position against one 16-byte cell per 512 — so it
    // is the one that most wants the advice. The fill below pushes exactly `block_count` entries
    // and the `truncate` at the end cannot reallocate, so the advice stays with the allocation it
    // was issued for.
    ...
    // The remaining fifth of the same structure; same argument as for `blocks` above.
