//! Throughput/memory measurement harness for the suffix-array index, at commit c4f0f30.
//!
//! # What this is
//!
//! A port of `sa-benchmarks/src/main.rs` from the `feature/preloaded-sa-improvements` branch, cut
//! down to what this commit can do. c4f0f30 is "Mappings.bin renamed to mapping.bin in releases" —
//! the commit that introduced the **initial, pre-optimisation mmap/preloaded split**, 32 commits
//! after fbc9328 and 174 before the branch tip. Measuring it says what the branch's optimisation
//! work bought on top of merely having the split.
//!
//! Companion to the fbc9328 port on `baseline/fbc9328-harness`. Same contract, same schema, same
//! staging mechanism; see that tree's README for the shared reasoning.
//!
//! # The schema is the contract
//!
//! `SCHEMA_VERSION`, every field of `BenchmarkRecord`, and the JSONL layout are IDENTICAL to the
//! branch's, so records from here drop into a branch session and land beside the branch's own arms
//! in one table. Do not "improve" a field.
//!
//! # Two arms, and they are all-or-nothing
//!
//! At this commit the storage choice is a **runtime bool** — `sa_server::load_*_file(path, mmap)` —
//! and ONE bool covers the suffix array, the proteins and the mapping together. There is no
//! per-structure selection, so `pprot` / `ptext` / `pmap` have no counterpart here and never will:
//! the ability to mix is itself something the branch added. Two arms exist, `mmap` and `preloaded`,
//! and the four `*_BACKEND` constants therefore always agree with each other.
//!
//! The bool is fed from `cfg!(feature = "mmap")` rather than a CLI flag. That is not a change to
//! what is measured — the same two configurations, selected the same way at the same point — but
//! the driver picks arms by cargo feature and refuses two arms that compile to the same bytes, and
//! this gives it two binaries that genuinely differ.
//!
//! # What this commit does not have, and what was done about it
//!
//! * **No k-mer bound table.** The axis is gone, `kmer_k` pinned to 0, and a grid cell asking for a
//!   table is REJECTED rather than run untabled. Compare against the branch's `kmer_k = 0` cells.
//! * **No batched search.** No `search_all_matching_suffixes_batched` and no MLP interleaving;
//!   phase 1 is the flat `par_iter` this commit's own `search_all_peptides` uses.
//! * **No `frame_chunks` / `json_chunk`, and an owned `ProteinInfo`.** Reimplemented below with
//!   identical framing. This commit clones the accession and decodes the annotations into an owned
//!   `String` per hit, where the branch borrows both — a real difference the branch made, and one
//!   the `response` phase is there to show.
//! * **No `touch_all_pages`.** The branch sweeps pages through `memory_hints`, which does not exist
//!   here and whose mmap handles are private anyway. Replaced by an accessor-driven sweep — see
//!   `warmup_touch_pages`, which also says what that costs the `GB/s` column.
//! * **No `ActiveSearcher`.** `Searcher::new(sa, proteins, mapping)` is already the branch's exact
//!   signature; only the loaders differ, by taking the mmap bool.
//!
//! # The index is read unchanged
//!
//! `sa.bin`, `proteins.bin` and `mapping.bin` all load directly from the branch's index directory,
//! in both arms — verified by answering 2,000 peptides identically to the branch. Nothing is
//! rebuilt and nothing is converted.
//!
//! ```text
//! cargo build --release -p sa-benchmarks                  # preloaded arm
//! cargo build --release -p sa-benchmarks --features mmap  # mmap arm
//! ```
//!
//! See `sa-benchmarks/README.md` for the runbook.
...
// ---------------------------------------------------------------------------
// The storage arm, which at this commit is one decision for everything
// ---------------------------------------------------------------------------

/// Whether this build maps the index or reads it into owned memory.
///
/// The branch exports four independent `*_BACKEND` constants because it can mix; this commit takes
/// one bool and hands the same value to all three loaders, so the four below are the same string by
/// construction. That is the honest shape of the arm, and it is why `pprot` / `ptext` / `pmap` have
/// no counterpart here.
const USE_MMAP: bool = cfg!(feature = "mmap");

const BACKEND: &str = if USE_MMAP { "mmap" } else { "preloaded" };
const SA_BACKEND: &str = BACKEND;
const TEXT_BACKEND: &str = BACKEND;
const PROTEINS_BACKEND: &str = BACKEND;
const MAPPING_BACKEND: &str = BACKEND;

fn backend_summary() -> String {
    format!("sa={BACKEND} text={BACKEND} proteins={BACKEND} mapping={BACKEND} (one runtime flag, not four)")
}

/// The branch has an `ActiveSearcher` whose storage is chosen per structure at compile time. Here
/// there is one `Searcher` and one flag, so the alias exists only to keep the two ports' code
/// readable side by side.
type ActiveSearcher = Searcher;
...
/// the annotations into an owned `String` per hit, where the branch borrows both and streams the
/// decode straight into the serialiser. That difference is a change the branch made, so it belongs
/// in the measurement rather than being papered over.
...
struct Args {
    ...
    // No `--kmer-table-file` / `--build-kmer-table`: the k-mer bound table does not exist at this
    // commit. A grid cell asking for one is rejected in `load_grid_file` rather than ignored, so a
    // suite cannot quietly report table-accelerated cells that ran without a table.
    ...
    /// is directly comparable with a `/search` response from either tree. Used by the answer gate.
    #[arg(long)]
...
fn run_benchmark(
    ...
    // `search_all_matching_suffixes_batched` and the `par_chunks(MLP_BATCH)` interleaving are things
    // the branch added — so this is the decomposition this tree's own production path uses:
...
fn print_dry_run(args: &Args) -> Result<(), Box<dyn Error>> {
    ...
    println!("Run once per backend (preloaded, mmap) — this commit has exactly those two.");
    Ok(())
}
...
/// Bytes a 4 KiB page holds, for stepping an accessor across one entry per page.
...
/// Touches every page of every mapped region, populating the page cache and the page tables.
///
/// # How this differs from the branch's, and what that costs
///
/// The branch calls `memory_hints::warmup::touch_all_pages`, which walks the raw `Mmap` one byte
/// per page under `MADV_SEQUENTIAL`. Neither exists here: there is no `memory-hints` crate at this
/// commit, and the mmap handles inside `SuffixArray` / `ProteinText` / the mapping are private, so
/// there is nothing to hand it even if there were.
///
/// This sweeps through the public accessors instead — one `get` per page-worth of entries — which
/// faults in exactly the same pages and populates the same PTEs. Two consequences, both real:
///
///   * **`GB swept` is comparable with the branch's**, because it is computed analytically from the
///     same entry counts and widths rather than measured.
///   * **`GB/s` is NOT comparable with the branch's.** An accessor call per page is far more work
///     than a byte load per page, and there is no `MADV_SEQUENTIAL` to help the readahead. Read this
///     arm's sweep RATE against the other c4f0f30 arm, never against the branch's. What the column
///     is for — telling a cold sweep from one handed a warm page cache — still works within a
///     commit, which is the comparison it was added for.
///
/// The preloaded arm sweeps nothing and reports zero, exactly as the branch's does, because at this
/// commit nothing is mapped in that build at all.
fn warmup_touch_pages(searcher: &ActiveSearcher) -> (SweepStat, SweepStat, SweepStat) {
    if !USE_MMAP {
        return (SweepStat::default(), SweepStat::default(), SweepStat::default());
    }
    ...
    rayon::scope(|scope| {
        scope.spawn(|_| {
            ...
            let (len, bits) = (searcher.sa.len(), searcher.sa.bits_per_value());
            for index in (0..len).step_by(stride_for(bits)) {
                std::hint::black_box(searcher.sa.get(index));
            }
            sa = SweepStat { bytes: (len as u64 * bits as u64).div_ceil(8), ms: start.elapsed().as_millis() as u64 };
        });
        scope.spawn(|_| {
            let start = Instant::now();
        ...
        scope.spawn(|_| {
            let start = Instant::now();
            // Driven over text positions rather than over the mapping's own layout, which differs
            // per style and is not public. For the bitvec mapping the current index uses, that is a
            // linear walk of one bit per position, so a page-worth of positions is a page.
            let positions = searcher.proteins.text().len();
            for index in (0..positions).step_by(stride_for(1)) {
                std::hint::black_box(searcher.suffix_index_to_protein.suffix_to_protein(index as i64));
            }
            mapping = SweepStat { bytes: (positions as u64).div_ceil(8), ms: start.elapsed().as_millis() as u64 };
        });
    });
...
fn dump_answers(searcher: &ActiveSearcher, args: &Args, out: &Path) -> Result<(), Box<dyn Error>> {
    ...
    eprintln!(
        "Answering {} peptides (cutoff {}, equate_il {}, tryptic {})...",
        peptides.len(),
        args.max_matches,
        args.equate_il,
        args.tryptic
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let t0 = Instant::now();
    let suffix_array = load_suffix_array_file(sa_path.to_str().unwrap(), USE_MMAP)?;
    ...
    let t0 = Instant::now();
    let proteins = load_proteins_file(proteins_path.to_str().unwrap(), USE_MMAP)?;
    ...
    let t0 = Instant::now();
    // Destructured: at this commit the loader hands back a newtype around the boxed trait object,
    // where the branch's returns the mapping itself.
    let SuffixToProteinMapping(mapping) = load_mapping_file(mapping_path.to_str().unwrap(), USE_MMAP)?;
